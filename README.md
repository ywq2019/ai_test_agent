...
| AI 生成用例 | 支持三种输入模式：粘贴 Swagger/OpenAPI 文档、自然语言描述、**直接粘贴接口实现代码**（Python/Java/Go/Node.js/PHP），AI 解析代码入参、边界、异常路径自动生成覆盖 P0/P1/P2 的用例 |
| **代码可行性分析** | 同时粘贴需求文档和接口实现代码，AI 对比两者识别四类偏差：`missing`（需求有但代码未实现）、`mismatch`（行为不一致）、`extra`（代码额外隐式限制）、`risk`（潜在代码风险），每条偏差含严重程度、需求描述、代码行为、测试重点、修复建议，并自动生成差异验证用例可一键保存到用例库 |
...
| POST | `/api/v1/api-test/projects/{id}/cases/generate` | AI 生成接口用例（Swagger/描述/代码三种输入模式） |
| POST | `/api/v1/api-test/projects/{id}/cases/generate-from-code` | 从接口实现代码直接生成用例 |
| POST | `/api/v1/api-test/projects/{id}/code-analyze` | 代码可行性分析（需求文档 vs 实现代码差异对比） |
| POST | `/api/v1/api-test/projects/{id}/code-analyze/save-cases` | 将差异验证用例保存到用例库 |
