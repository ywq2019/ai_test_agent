...
> **浏览器说明**：Docker 镜像基于官方 `mcr.microsoft.com/playwright/python:v1.39.0-jammy`，Chromium 已内置，无需额外安装。**本地开发（Windows）** 需手动安装并配置 `PLAYWRIGHT_BROWSERS_PATH` 指向浏览器目录（`.env` 文件已有示例）。
...
Playwright 驱动浏览器，以「AI 场景规划 → 录制 → 健壮化 → 执行」为主线，用最少的脚本维护成本覆盖完整 UI 测试流程。

| 功能 | 说明 |
| --- | --- |
| **AI 场景规划** | 分析页面元素和需求文档，按核心流程/表单验证/增删改/筛选/异常反馈自动规划录制场景，支持追加/重新规划 |
| **录制 → 场景联动** | 场景规划结果在抽屉中展示，点击「开始录制」直接启动有头浏览器；录制完成自动标记场景为已录制 |
| **录制健壮化** | 保存时自动推导多候选 selector（A/B/C/D 稳定性评级），关键操作后插入 wait + assert，降低执行失败率 |
| **可视化步骤编辑器** | 三 Tab 弹窗（基本信息 / 步骤编辑器 / 前置步骤）；行内编辑 action/selector/value/expected；D 级 selector 标红；点击胶囊展开候选列表一键切换 |
| **前置步骤** | 用例级前置操作（导航/点击/填写），与主步骤共用同一 Page；前置失败则用例直接标为 failed |
| **登录态快照** | 为任务指定登录用例，批量执行前自动运行一次并保存 `storage_state`；后续每条用例直接恢复登录态，零耗时建立会话；支持 TTL 自动刷新 |
| **AI 生成用例** | 配置面板支持自然语言测试重点、指定模块、期望数量；服务端用页面真实 selector 而非 LLM 自造；按优先级硬截断到目标数量 |
| **执行状态** | 用例列表内联显示上次执行结果（通过/失败/错误原因）；实时进度浮层（右下角，非阻塞）展示步骤轨道和用例计数 |
| **多 Selector 回退** | 执行时逐候选探测，找到可用元素即执行，应对动态 id / 框架重构 |
